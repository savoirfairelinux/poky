...
NPM_ARCH ?= "${@npm_arch(d.getVar('TARGET_ARCH'))}"
...
npm_do_compile() {
    ...
    # needs to be cleared. But to not mess with the local cache, the npm
    # config needs to be updated to use another cache directory. And to not
    # mess with the local '~/.npmrc` file, the HOME needs to be updated as well.
    HOME=${WORKDIR}
